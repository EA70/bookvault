 ...
 */
 const dev_sendVerificationEmail = async (studentEmail, studentName, token) => {
...
const sendVerificationEmail = async (studentEmail, studentName, token) => {
  ...
  await resend.emails.send({
    from: 'BuchVault <onboarding@resend.dev>', // ← fonctionne sans domaine perso
    to: studentEmail,
    subject: "Activer votre compte BuchVault",
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
        ...
        <div style="text-align: center; margin: 35px 0;">
          <a href="${verificationLink}" style="background-color: #7c3aed; color: #ffffff; padding: 14px 32px; text-decoration: none; border-radius: 10px; font-weight: bold; font-size: 14px; display: inline-block;">
            Activer mon compte
          </a>
        </div>
 ...
 */
const dev_sendResetPasswordEmail = async (studentEmail, resetLink) => {
  const mailOptions = {
...
const sendResetPasswordEmail = async (studentEmail, resetLink) => {
  await resend.emails.send({
    from: 'BuchVault Sécurité <onboarding@resend.dev>',
    to: studentEmail,
    subject: "Réinitialisation de votre mot de passe - BuchVault",
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
